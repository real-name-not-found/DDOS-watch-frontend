        ...
        return (
            <div className="max-w-4xl mx-auto text-center">
                <div className="flex items-center justify-center gap-3 mb-6">
                    <span className="text-[10px] uppercase tracking-widest font-medium text-typo-text-light">{eyebrow}</span>
                    <span className="w-14 h-px bg-typo-text"></span>
                </div>
            ...
            <motion.section {...fadeIn} transition={{ delay: 0.2 }}>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 mt-8">
                    <div className="xl:col-span-7 border border-typo-border bg-white p-6 md:p-8">
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
                            <div>
                                <p className="text-[10px] uppercase tracking-widest text-typo-text-light mb-3">Held-out evaluation</p>
                                <h3 className="text-4xl md:text-5xl font-serif text-typo-text leading-none">Benchmark frame</h3>
                            </div>
                            <PillRail items={['Accuracy', 'ROC-AUC', 'Average precision', 'Brier score']} activeIndex={1} />
                        </div>

                        <div className="border border-typo-border bg-typo-bg/40 p-5 md:p-8">
                            <div className="grid grid-cols-3 gap-6 items-end min-h-[320px]">
                                {modelPageData.modelComparison.map((entry) => {
                                    const normalized = maxRoc === minRoc ? 1 : (entry.rocAuc - minRoc) / (maxRoc - minRoc);
                                    const height = 42 + normalized * 34;
                                    const accent = entry.model === 'XGBoost'
                                        ? 'bg-typo-text'
                                    ...
                                    return (
                                        <div key={entry.model} className="flex flex-col items-center text-center">
                                            <p className="text-2xl font-serif text-typo-text mb-3">{(entry.rocAuc * 100).toFixed(2)}</p>
                                            <div className="w-full h-[220px] flex items-end justify-center">
                                                <div
                                                    className={`w-full max-w-[140px] rounded-t-[18px] ${accent}`}
                                                    style={{ height: `${height}%` }}
                                                />
                                            </div>
                                            <h4 className="mt-4 text-2xl font-serif text-typo-text">{entry.model}</h4>
                                            <p className="mt-2 text-[10px] uppercase tracking-widest text-typo-text-light">{entry.status}</p>
                                        </div>
                                    );
                                })}
                            </div>

                            <p className="mt-6 text-sm leading-7 text-typo-text-light">
                                The benchmark card emphasizes ROC-AUC because it best captures how well the model separates malicious from low-risk infrastructure. XGBoost finished first while also keeping the best Brier score.
                            </p>
                        </div>
                    </div>

                    <div className="xl:col-span-5 border border-typo-border bg-white p-6 md:p-8">
                        <p className="text-[10px] uppercase tracking-widest text-typo-text-light mb-6">Why XGBoost was retained</p>
                        <div className="space-y-5">
                            <div className="border-t border-typo-line pt-5 first:border-t-0 first:pt-0">
                                ...
                                <p className="text-sm leading-7 text-typo-text-light">
                                    XGBoost produced the strongest held-out ROC-AUC at <span className="font-medium text-typo-text">0.9882</span>, outperforming both the tree baseline and the linear baseline on the main ranking metric.
                                </p>
                            </div>
                            <div className="border-t border-typo-line pt-5">
                                <h3 className="text-2xl font-serif text-typo-text mb-2">Best probability quality</h3>
                                <p className="text-sm leading-7 text-typo-text-light">
                                    Its Brier score of <span className="font-medium text-typo-text">0.0354</span> was the lowest of the tested models, so it was not only ranking well but also calibrating more usefully.
                                </p>
                            </div>
                            <div className="border-t border-typo-line pt-5">
                                <h3 className="text-2xl font-serif text-typo-text mb-2">Stable after extra checks</h3>
                                <p className="text-sm leading-7 text-typo-text-light">
                                    After grouped cross-validation and compact tuning, the baseline XGBoost configuration remained strong enough to retain without chasing marginal gains.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="border border-typo-border bg-white mt-6 overflow-x-auto">
                    <table className="w-full min-w-[860px] text-left">
                        <thead>
                            <tr className="border-b border-typo-border">
                                ...
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-typo-text-light font-medium">Accuracy</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-typo-text-light font-medium">ROC-AUC</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-typo-text-light font-medium">Average precision</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-typo-text-light font-medium">Brier score</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-typo-text-light font-medium">Role</th>
                            </tr>
                        </thead>
                        <tbody>
                            {modelPageData.modelComparison.map((entry) => (
                                <tr key={entry.model} className={`border-b border-typo-line last:border-b-0 ${entry.model === 'XGBoost' ? 'bg-typo-bg/50' : 'bg-white'}`}>
                                    <td className="px-6 py-5 text-xl font-serif text-typo-text">{entry.model}</td>
                                    <td className="px-6 py-5 text-sm text-typo-text">{entry.accuracy.toFixed(4)}</td>
                                    <td className="px-6 py-5 text-sm text-typo-text">{entry.rocAuc.toFixed(4)}</td>
                                    <td className="px-6 py-5 text-sm text-typo-text">{entry.averagePrecision.toFixed(4)}</td>
                                    <td className="px-6 py-5 text-sm text-typo-text">{entry.brierScore.toFixed(4)}</td>
                                    <td className="px-6 py-5 text-[10px] uppercase tracking-widest text-typo-text-light">{entry.status}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            <motion.section {...fadeIn} transition={{ delay: 0.25 }}>
                ...
                <div className="border border-typo-border bg-white mt-6 overflow-x-auto">
                    <div className="px-6 py-5 border-b border-typo-border flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            <p className="text-[10px] uppercase tracking-widest text-typo-text-light mb-2">Compact tuning leaderboard</p>
                            <h3 className="text-3xl font-serif text-typo-text">Grouped CV search around the baseline</h3>
                        </div>
                        <PillRail items={['6 nearby configs', 'Grouped CV', 'Baseline retained']} activeIndex={2} />
                    </div>

                    <table className="w-full min-w-[980px] text-left">
                        <thead>
                            <tr className="border-b border-typo-border">
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-typo-text-light font-medium">Config</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-typo-text-light font-medium">Trees</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-typo-text-light font-medium">Depth</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-typo-text-light font-medium">LR</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-typo-text-light font-medium">Mean accuracy</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-typo-text-light font-medium">Mean ROC-AUC</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-typo-text-light font-medium">Mean AP</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-typo-text-light font-medium">Mean Brier</th>
                                <th className="px-6 py-4 text-[10px] uppercase tracking-widest text-typo-text-light font-medium">Decision</th>
                            </tr>
                        </thead>
                        <tbody>
                            {modelPageData.validation.tuningLeaderboard.map((row) => (
                                <tr key={row.config} className={`border-b border-typo-line last:border-b-0 ${row.retained ? 'bg-typo-bg/55' : 'bg-white'}`}>
                                    <td className="px-6 py-5 text-base font-serif text-typo-text">{row.config}</td>
                                    <td className="px-6 py-5 text-sm text-typo-text">{row.nEstimators}</td>
                                    <td className="px-6 py-5 text-sm text-typo-text">{row.maxDepth}</td>
                                    <td className="px-6 py-5 text-sm text-typo-text">{row.learningRate.toFixed(2)}</td>
                                    <td className="px-6 py-5 text-sm text-typo-text">{row.meanAccuracy.toFixed(4)}</td>
                                    <td className="px-6 py-5 text-sm text-typo-text">{row.meanRocAuc.toFixed(4)}</td>
                                    <td className="px-6 py-5 text-sm text-typo-text">{row.meanAveragePrecision.toFixed(4)}</td>
                                    <td className="px-6 py-5 text-sm text-typo-text">{row.meanBrierScore.toFixed(4)}</td>
                                    <td className="px-6 py-5 text-[10px] uppercase tracking-widest text-typo-text-light">
                                        {row.retained ? 'Retained final config' : 'Candidate'}
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                            ...
                            {topImportance.map((feature, index) => (
                                <div key={feature.key}>
                                    ...
                                    <div className="h-2 bg-typo-line/80 overflow-hidden">
                                        <div className="h-full bg-typo-text" style={{ width: `${(feature.value / maxImportance) * 100}%` }} />
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <motion.section {...fadeIn} transition={{ delay: 0.4 }}>
                ...
                <div className="border border-typo-border bg-white p-6 mt-8">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                        <div>
                            <p className="text-[10px] uppercase tracking-widest text-typo-text-light mb-2">Probability-to-band mapping</p>
                            <h3 className="text-3xl font-serif text-typo-text">Operational thresholds</h3>
                        </div>
                        <PillRail items={['Low risk', 'Monitor', 'High risk']} activeIndex={1} />
                    </div>
            ...
            <motion.section {...fadeIn} transition={{ delay: 0.45 }}>
                ...
                <p className="mt-6 text-sm md:text-base leading-8 text-typo-text-light">
                    {body}
                </p>
            </div>
        );
    ...
    return (
        <div className={`flex flex-wrap items-center gap-3 ${className}`}>
            {items.map((item, index) => (
                <span
                    key={item}
                    className={`px-4 py-2 rounded-full border text-xs uppercase tracking-widest ${
                        index === activeIndex
...
export default function IntelligenceLaunch() {
    const fadeIn = {
    ...
    return (
        <div className="w-full max-w-[1600px] mx-auto px-6 py-12 flex flex-col gap-16">
